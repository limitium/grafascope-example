#!/usr/bin/env bash
# Test OTLP gRPC ingress (external call to domain/grafascope/victoria-traces-grpc).
# Verifies the ingress routes gRPC and returns a valid response.
#
# Usage: ./scripts/test-otlp-grpc-ingress.sh [HOST:PORT] [PATH_PREFIX]
...
echo "Testing OTLP gRPC ingress at $URL ..."
...
if [[ "$HTTP_CODE" == "200" || "$HTTP_CODE" == "400" ]]; then
  echo "OK: Ingress is routing (HTTP $HTTP_CODE). gRPC endpoint is reachable."
  echo "    Use endpoint URL: http://${HOST_PORT}${PATH_PREFIX} for OTLP gRPC clients that support path."
  exit 0
else
  echo "FAIL: Got HTTP $HTTP_CODE (expected 200 or 400). Check ingress and namespace."
  exit 1
fi
